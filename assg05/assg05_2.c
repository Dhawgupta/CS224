...
student enter_data() // Entering the data
{
	student t;
...
{	
	...
	{
		y = x;
		int i= strcmp(t->rollno,x->rollno);
		if(i<0)	
			x = x->left;
		else if(i>0) 
			x = x->right;
...
void tree_transplant(student *u,student *v)
{
	if (u->parent == NULL)
		stud_top = v;
...
{
	if(p->left == NULL)
		tree_transplant(p,p->right);
	else if(p->right == NULL) tree_transplant(p,p->left);
	else
	{
		student* y = tree_min(p->right);
		if(y->parent != p)
		{
