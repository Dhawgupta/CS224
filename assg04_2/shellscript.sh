...
QSORT_FILE_1="assg04b_1.c"
QSORT_FILE="assg04b_2.c"
RAND_GEN_FILE="randInput2.c"
...
rm -f $datafile
rm -f $gnuplotdatafile

#compile QSORT_FILE
$CC $QSORT_FILE_1 -g -o $QSORT_EXE_1 -lm
...
do
	...
	do
		...
		/usr/bin/time --format="EXECUTION_TIME: %U"  -o $outFile  ./$QSORT_EXE_1 < $inputfile
		#echo "time 1 found done"

		#take the execution time
		exeTime=`awk '/^EXECUTION_TIME/ {print $2}' $outFile`
		#echo "exeTime finding done"

		#Storing the time in a file
		printf ":%s" $exeTime >> $datafile

	    #echo Repeating the same steps as above when the pivot is chosen randomly.
		
		
		/usr/bin/time --format="EXECUTION_TIME: %U"  -o $outFile  ./$QSORT_EXE < $inputfile
		
		#echo take the execution time
		exeTime=`awk '/^EXECUTION_TIME/ {print $2}' $outFile`
		#echo exetime done
		printf ":%s" $exeTime >> $datafile
		
		iteration=`expr $iteration + 1`
		#echo changing iteration done $iteration
		count=`expr $count + 1`
	done
