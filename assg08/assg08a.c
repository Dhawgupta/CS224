...
#include <stdlib.h>
#define INT_MAX 1<<31
struct adjlist // structre for a node
{
	char word[6];
	int index;
	int visit;
...
void build_graph(struct adjlist *nodes,int **adjm,int n); // function to build graph
void print_shortest_path(struct adjlist *nodes,int n,int **adjm,int i1,int i2); // function to find the shortest path
int calc_cost(char *word1 , char *word2); // to calculate cost between 2 words
int main()
{
	printf(">");
	int n;
...
void min_heapify(struct adjlist *nodes,int *heap,int i, int heap_size) // function to heapify a ceratin node of th heap 
{
	int l,r;
	l = left(i);
	r = right(i);
	...
	if(smallest !=i)
	{
		...
		heap[smallest] = temp;
		min_heapify(nodes,heap,smallest,heap_size);
	}	
}
void build_heap(struct adjlist *nodes,int *heap,int heap_size)// function to build heap
{
	int i=0;
	for(i=heap_size/2;i>=1;i--)
		min_heapify(nodes,heap,i,heap_size);
}

void print_path(struct adjlist * nodes, int n1,int n2)// print path recursively
{
	if (n1 == n2)
	{
		printf("\n%s",nodes[n1].word);
...
void print_shortest_path(struct adjlist *nodes,int n,int **adjm,int i1,int i2) // find the shortest path
{
	int i=0;
	for(i=0;i <n;i++){
	...
	while(heap_size >0  && heap[1] != i2)
	{
		...
		//min_heapify(nodes,heap,1,heap_size-1);
		heap_size--;
		for(i=0;i<n;i++)
		{
...
int check_difference(char *a,char *b) // check the difference if 2 nodes are adjacent to each other
{
	int flag =0;
	int i=0;
	for(i=0;i<5;i++)
...
void build_graph(struct adjlist *nodes,int **adjm,int n) // build the graph structure i.e. the adjacen matrix
{
	int i,j;
	for(i=0;i<n;i++)
	{
